...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 9
            #错的：if ContainerAnt.can_contain(self, place.ant) and isinstance(self, ContainerAnt):
            #因为self是新来的，所以在can_contain中无法判断“if self.contained_ant==None:”
            if not isinstance(place.ant, ContainerAnt) and isinstance(self, ContainerAnt):
                ContainerAnt.contain_ant(self, place.ant)
                place.ant=self
            elif isinstance(place.ant, ContainerAnt) and ContainerAnt.can_contain(place.ant, self):#place.ant.contained_ant==None and not isinstance(self, ContainerAnt)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        temp_range=0
        temp_range=self.min_range
        position=self.min_range
        while position>0 and temp_place is not beehive:
            temp_place=temp_place.entrance
            position-=1
        #if position>0 or temp_place is beehive:
        #    return None#有两次temp_place is not beehive的判断无需再多此一举
        #while temp_range<self.min_range and temp_place is not beehive:
        #    temp_range+=1
        #    temp_place=temp_place.entrance
        #if temp_range<self.min_range:
        #    return None
        while temp_range<=self.max_range and temp_place is not beehive:
            if rANTdom_else_none(temp_place.bees)!=None:
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        if self.index!=0:
            ...
            temp_place=self.place
            while temp_place.exit:#!=None None可以作为False来用
                if temp_place.exit.ant!=None and not temp_place.exit.ant.buffed:
                    temp_place.exit.ant.damage=2*temp_place.exit.ant.damage
                    temp_place.exit.ant.buffed=True#做标记的常用方法
                if isinstance(temp_place.exit.ant, ContainerAnt) \
                and temp_place.exit.ant.contained_ant!=None\
                and not temp_place.exit.ant.contained_ant.buffed:
                    temp_place.exit.ant.contained_ant.damage=2*temp_place.exit.ant.contained_ant.damage
                    temp_place.exit.ant.contained_ant.buffed=True
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor==0 and self.index==0:
            bees_win()
