...
def is_bst(t):
    ...
    if t.branches==[]:
        ...
        if left.label<=t.label:
            if bst_max(left)<=t.label:
                return is_bst(left)
            else:
                return False
        else:
            if bst_min(right)>t.label:
                return is_bst(right)   
            else:
                return False  
...
def path_yielder(t, value):
    ...
    for b in t.branches:
        ...
        for ls in path_yielder(b, value):
            yield [t.label]+ls
...
#在你当前的实现中，重新分配 link 变量不会影响函数外部的原始链表。

#为了正确地从链表中删除节点，你应该修改前一个节点的 rest 指针，以绕过具有指定值的节点。 “mutate”
