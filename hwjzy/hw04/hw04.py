...
def totals_tree(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    label_t=total_weight(m)
    if is_planet(m):
        return tree(label_t)
    return tree(label_t, [totals_tree(end(left(m))), totals_tree(end(right(m)))])
...
def replace_leaf(t, find_value, replace_value):
    ...
    if is_leaf(t1):
        if label(t1)==find_value:
            t1=tree(replace_value) 
    else:
        a=[]
        for b in branches(t1):
            a.append(replace_leaf(b, find_value, replace_value))
        t1=tree(label(t), branches=a)
    return t1
...
def profit(t):
    ...
    >>> t=tree(2,[tree(3,[tree(5,[tree(9)]),tree(6,[tree(10),tree(11)])]), 
    ...           tree(4, [tree(7),tree(8,[tree(12)])])])
    >>> profit(t)
    52 
